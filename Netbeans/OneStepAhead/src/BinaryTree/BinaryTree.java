...
public class BinaryTree {
    ...
    void minMax(Node node) {
        int min, max; //create variable for place
        min = Min(node); //call min function
        max = Max(node); //call max function
        System.out.println("Minimum value : " + min); //print
        System.out.println("Maximum value : " + max); //print
    }

    int Min(Node node) {
        int min; //create variable
        if (node.left == null) { //if node left is null
            return node.data; //return the data
        } else { //if not null
            return Min(node.left); //recursive to traverse to left node
        }
    }

    int Max(Node node) {
        int max; //create variable
        if (node.right == null) { //if node right is null
            return node.data; //return the data
        } else { //if not null
            return Max(node.right); //recursive to traverse to right node
        }
    }
